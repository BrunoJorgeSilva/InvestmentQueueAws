...
public class InvesmentServiceTests
{
    ...
    public async Task Invest_ShouldReturnSuccess_WhenInvestmentIsSuccessful()
    {
        ...
        _mockLogger.Verify(
            x => x.Log(
                ...
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString().Contains($"Starting to Invest {investmentDto.ClientId}")),
                null,
    ...
    public async Task Invest_ShouldReturnFailure_WhenInvestmentFails()
    {
        ...
        LogLevel.Information,
        It.IsAny<EventId>(),
        It.Is<It.IsAnyType>((v, t) => v.ToString().Contains($"Result of the investment {investmentDto.ClientId}")),
        null,
        It.IsAny<Func<It.IsAnyType, Exception, string>>()),
        Times.AtLeastOnce);
    }
